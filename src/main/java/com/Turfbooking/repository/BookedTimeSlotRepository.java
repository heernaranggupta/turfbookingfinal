...
public interface BookedTimeSlotRepository extends MongoRepository<BookedTimeSlot, String> {
    ...
    BookedTimeSlot findByTurfIdAndStartTimeAndDate(String turfId, LocalDateTime startTime, LocalDate date);
    ...
    @Query("{'date' :  { $gte: ?0, $lte: ?1 }")
    List<BookedTimeSlot> findAllByDate(LocalDate fromDate, LocalDate toDate, String status);
    ...
    @Query("{'orderId': ?0}")
    List<BookedTimeSlot> findByOrderId(String orderId);

    @Query("{'_id':?0}")
    BookedTimeSlot findByIdEquals(String bookingId);

    @Query("{'date':{'$gt': ?0}}")
    List<BookedTimeSlot> findByDateIsAfter(LocalDate date);
